...
#define MOUNT_HELPER_RETRIES 10
#endif /* ANDROID */
		...
		const char *opts, int quiet)
{
	...
	while ((fm_socket = socket_local_client("vold_fconn", 1, SOCK_STREAM)) == -1) {
		ALOGE("looking for socket (%d)", fm_socket);
		sleep(1);
		tries++;
		if (tries >= MOUNT_HELPER_RETRIES) {
			ALOGE("Could not connect to mount helper after %d retries\n", tries);
			return -1;
		}
	}
	rv = receive_fd(fm_socket);
	close(fm_socket);
...
#ifdef ANDROID
	//Indicate external mount code path, handled by remote_vold.
	return -2;
#endif
