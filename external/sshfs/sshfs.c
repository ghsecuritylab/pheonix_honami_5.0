...
#define STDIN 0
#define STDOUT 1
static void start_proxy(int from_fd, int to_fd) {
	int bytes = 0, bytes_written = 0;
	char buf[256] = { 0, };
	while (1) {
		bytes = read(from_fd, buf, sizeof(buf));
		if (bytes > 0) {
			bytes_written = write(to_fd, buf, bytes);
...
static void *stdin_thread_runner(void* pty_master)
{
	int pty_fd = (int)*((int*)pty_master);
	if (pty_fd != -1)
		start_proxy(STDIN, pty_fd);
	return NULL;
}

static void *stdout_thread_runner(void* pty_master)
{
	int pty_fd = (int)*((int*)pty_master);
	if (pty_fd != -1)
		start_proxy(pty_fd, STDOUT);
	return NULL;
}
#endif /* ANDROID */
...
static int start_ssh(void)
{
	...
	int pid;
#ifdef ANDROID
	int pty_fd, pty_slave_fd, sfd, test_fd, err;
	pthread_t stdout_thread;
	pthread_t stdin_thread;

	/*
	 * Same test that is present in ssh: if we can't open /dev/tty
	 * we were launched in a way that needs to use a pty.
	 */
	test_fd = open("/dev/tty", O_RDWR);
	if (test_fd == -1) {
		pty_fd = pty_master(&ptyname);
		if (pty_fd == -1)
			return -1;

		pty_slave_fd = open(ptyname, O_RDWR | O_NOCTTY);
		if (pty_slave_fd == -1)
			return -1;

		err = pthread_create(&stdout_thread, NULL,
				stdout_thread_runner, (void*)&pty_fd);
		if (err) {
			fprintf(stderr, "failed to create thread: %s\n",
					strerror(err));
		} else {
			err = pthread_create(&stdin_thread, NULL,
					stdin_thread_runner, (void*)&pty_fd);
			if (err) {
				fprintf(stderr, "failed to create thread: %s\n",
						strerror(err));
			}
...
#ifdef ANDROID
		setsid();
		/* open and close to start pty slave */
		sfd = open(ptyname, O_RDWR);
		if (sfd != -1) {
			close(sfd);
		}
...
#ifdef ANDROID
	/* normally you should wait for success here, */
	/* but due to some network occasions the event is never received */
	/* so we will perform a timed wait as a workaround */
	if ((sshfs.request_timeout > 0)
			&& (clock_gettime(CLOCK_REALTIME, &ts) == 0)) {
		/* wait semaphore for defined timeout period (sec) */
		ts.tv_sec += sshfs.request_timeout;
		do
			retval = sem_timedwait(&req->ready, &ts);
		while (retval == -1 && errno == EINTR);
		if (errno == ETIMEDOUT) {
			req->error = -errno;
			pthread_mutex_lock(&sshfs.lock);
			g_hash_table_remove(sshfs.reqtab,
...
static void random_string(char *str, int length)
{
	...
	for (i = 0; i < length; i++)
#ifdef ANDROID
		*str++ = (char)('0' + nrand48(sshfs.randseed) % 10);
#else
...
#ifdef ANDROID
"    -o request_timeout=N    sets timeout for a file system request in seconds (default: 60)\n"
#endif /* ANDROID */
"\n", progname);
}
...
int main(int argc, char *argv[])
{
	...
	sshfs.randseed[0] = (now >> 0) & 0xFFFF;
	sshfs.randseed[1] = (now >> 16) & 0xFFFF;
	sshfs.randseed[2] = (now >> 32) & 0xFFFF;
#else
...
#ifdef ANDROID
	tmp = g_strdup_printf("-osession_identifier=%s", sshfs.host);
